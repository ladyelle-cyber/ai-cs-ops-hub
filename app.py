...
with tabs[1]:
    ...
    with dtabs[1]:
        ...
        rdesc = st.text_area("What changed?", value="New feature that surfaces real-time patient-level care gaps ranked by closure probability and estimated revenue impact per closure. Integrates claims data directly into provider daily worklists — gaps appear where physicians already work, not in a separate platform. Industry benchmark: average time to close a gap via manual outreach is 47 days; this feature targets 12 days by routing the right gap to the right provider at the right time. Each Star rating point is worth approximately $500–$1,500 PMPM in MA bonus payments — this feature is the most direct lever for moving that number. Designed for quality directors and VP-level clinical leaders managing HEDIS measurement windows and CMS reporting timelines.", height=150, key="e2")
    ...
    with dtabs[2]:
        ...
        scen = st.text_area("Customer scenario", value="Meeting Wednesday with VP of Quality at Cornerstone Medical Group — 185-physician multispecialty practice in Phoenix, 9 months post-implementation. Platform adoption at 31%, well below our 60% benchmark at this stage. HCC coding completion at 61%. They're tracking $340K below their shared savings target with 90 days left in a $2.1M Aetna arrangement. COO initiated a vendor audit last month after CFO flagged 3 overlapping tools in their stack. My champion is the Quality Director but she reports to the COO, who I've never met. Sales flagged that Innovaccer ran a demo with their team last week.", height=150, key="d1")
        d1,d2 = st.columns(2)
        ...
        with d2: urg = st.selectbox("Urgency", ["Standard","At-risk","Competitive threat","Escalation"], index=2, key="d3")
    ...
    with dtabs[3]:
        ...
        obs = st.text_area("Field Observations", value="- 5 of 11 accounts this quarter report HCC coding completion below 65% — our top performers are at 82%+; gap is in workflow integration, not training\n- Lost 2 deals this month to point solutions priced at flat $3–5 PMPM vs. our per-module model — CFOs choosing predictable cost over capability breadth\n- 4 customers asked about ambient documentation in the last 30 days, all comparing to Nuance DAX; 2 said it is now a condition of their physician adoption plan\n- NPS last quarter: enterprise accounts (500+ providers) scoring 42, mid-market (100–250) scoring 31 — 11-point gap not explained by implementation differences\n- 3 customers with Q3 payer contract renewals requested ROI documentation; average contract value at risk across these accounts is $1.8M\n- Health system onboarding averaging 18 weeks vs. 6 weeks for independent practices — no standardized playbook for health system go-lives\n- 2 CMOs used the phrase 'reporting tool' when describing us to their CFOs — revenue driver positioning is not landing at the executive level", height=220, key="f1")
